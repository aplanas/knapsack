...
mask = (pyinotify.IN_DELETE |
        pyinotify.IN_CREATE |
        pyinotify.IN_MOVED_FROM)  # watched events
...
def find_packages(_prefixname):
    """Use the filename_prefix to find real packages. This assumes the
    prefices are sorted when called
...
if __name__ == '__main__':
    ...
    args.srcalt = os.path.abspath(args.srcalt) if args.srcalt else None
            ...
            try:
                ...
                if args.srcalt:
                    ...
                    try:
                        ...
                        # the initial startup is slow, so it happens that
                        # files disappear but once we're done here, we look
                        # at the notify events
                        pass
    ...
    print "looping"
    while args.lists not in changed_dirs:
        dirtoupdate = None
        change_lock.acquire()
        for dir in changed_dirs:
            if time.time() - changed_dirs[dir] < 100:
                continue
        ...
        if dirtoupdate not in all_dirs:
